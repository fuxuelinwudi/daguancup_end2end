...
class NeZhaForPreTraining(NeZhaPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and sentence_span_labels is not None:
            loss_fct = CrossEntropyLoss()
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))

            next_sentence_loss = loss_fct(seq_relationship_score.view(-1, 2), sentence_span_labels.view(-1))

            pseudo_labels = torch.argmax(torch.softmax(seq_relationship_score, -1), 1)
            pseudo_loss = loss_fct(seq_relationship_score.view(-1, 2), pseudo_labels.view(-1))
            next_sentence_loss = next_sentence_loss + 0.5 * pseudo_loss
